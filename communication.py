...
class CommunicationChannel:
    ...
    def establish_connection(self):
        ...
        print(f"[{self.role}] Private key: {private_key}, Public key: {public_key}")
        self.send_queue.put(public_key)
        print(f"[{self.role}] Sending public key to receiver: ",public_key)
        ...
        if self.role == 'sender':
            ...
            print(f"[{self.role}] Sending encrypted seed to receiver: ",encrypted_seed)
        else:
            rsa = RSA(key_size=64)
            rsa.generate_keys()
            ...
            print(f"[{self.role}] Received encrypted seed from sender: ",encrypted_seed)
        print(f"[{self.role}] Key exchange completed")
        return True
    ...
    def send_message(self, message):
        ...
        print(f"[{self.role}] Encrypting message...")
        cipher_text = ""
        ...
        print(f"[{self.role}] Message sent")
        return True
    ...
    def receive_message(self):
        ...
        print(f"[{self.role}] Waiting for message...")
        ...
        print(f"[{self.role}] Decrypting message...")
        decrypted_text = ""
    ...
    def close(self):
        ...
        print(f"[{self.role}] Channel closed")
